...
namespace kanan {
    ...
    string narrow(const string_view& str) {
        auto length = WideCharToMultiByte(CP_UTF8, 0, (wchar_t*)str.data(), (int)str.length(), nullptr, 0, nullptr, nullptr);
        string narrowStr{};

        narrowStr.resize(length);
        WideCharToMultiByte(
            CP_UTF8, 0, (wchar_t*)str.data(), (int)str.length(), (LPSTR)narrowStr.c_str(), length, nullptr, nullptr);
